...
class salesman_controller {
    ...
    static deleteSalesman = async (req, res) => {
        try {
            const salesman = await salesmanModel.findById(req.salesman.id)
            ...
            await salesmanModel.deleteOne({_id: req.salesman.id});

            res.status(200).send({
                apiStatus: true, message: "Salesman and his social performance records were deleted", data: salesman
            });
        } catch (e) {
            res.status(500).send({message: e.message, data: e});
        }
    ...
    static createSocialPerformanceToSalesman = async (req, res) => {
        try {
            ...
            res.status(200).send({message: "Social performance record successfully created"});
