const socialPerformanceRecordModel = require("../model/SocialPerformanceRecord")
const salesmanModel = require("../model/SalesMan");
...
class social_performance_record_controller {
    ...
    static getSocialPerformanceRecordById = async (req, res) => {
        try {
            const data = await socialPerformanceRecordModel.findById(req.params.id).populate("SalesMan")
            if (data == null) {
    ...
    static updateSocialPerformanceRecord = async (req, res) => {
        ...
        try {
            ...
            if (socialPerformanceRecordModel.findById(req.params.id) == null) {
                res.status(404).send({message: "Social Performance not found"});
            }
            ...
            res.status(200).send({message: "Social Performance was found", data: recordUpdated});
